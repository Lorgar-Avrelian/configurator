...
> [!TIP]
> Конфигурация опроса устройства, также как и конфигурация опроса по умолчанию, является указателем, связующим индикатор 
> устройства и структуру устройства, которая соответствует этому индикатору.  
> Таким образом, API из данного блока предоставляют типовой набор CRUD-операций для создания конфигураций опроса 
> устройств.
...
> устройства, либо связанной с конфигурацией структуры устройства будет удалена и соответствующая им конфигурация
> опроса.

> [!IMPORTANT]
> Рабочие конфигурации опроса устройств не имеют ограничений по количеству репликаций индикаторов устройств в
> отличие от конфигураций опроса по умолчанию. Более того, одному конкретному устройству соответствует одна конкретная
> рабочая конфигурация опроса.
...
```http
POST https://nms-dev.opk-bulat.ru/api/v1/catalog/configuration?indicator=2&device_component=1
...
```http
GET https://nms-dev.opk-bulat.ru/api/v1/configuration/1
...
```http
PUT https://nms-dev.opk-bulat.ru/api/v1/catalog/configuration/3?indicator=2&device_component=4
...
```http
DELETE https://nms-dev.opk-bulat.ru/api/v1/configuration/3
...
```http
GET https://nms-dev.opk-bulat.ru/api/v1/catalog/configurations
