...
> [!TIP]
> Конфигурация по умолчанию фактически является указателем, связующим индикатор устройства и структуру устройства, 
> которая соответствует этому индикатору.  
> Таким образом, API из данного блока предоставляют типовой набор CRUD-операций для создания конфигураций по умолчанию.

> [!WARNING]
> После создания связи между индикатором устройства и структурой устройства её реализация осуществляется с
> использованием ресурсов БД путём использования механизма внешних ключей. Таким образом, при удалении либо индикатора
> устройства, либо связанной с конфигурацией структуры устройства будет удалена и соответствующая им конфигурация опроса 
> по умолчанию.

> [!IMPORTANT]
> Конфигурации опроса по умолчанию имеют одно важное ограничение: одному индикатору устройства (который является 
> уникальным, то есть хранится для набора значений в только в одном экземпляре) может соответствовать только одна 
> конфигурация опроса по умолчанию.
...
```http
POST https://nms-dev.opk-bulat.ru/api/v1/catalog/default-configuration?indicator=2&device_component=1
...
```http
GET https://nms-dev.opk-bulat.ru/api/v1/catalog/default-configuration/1
...
```http
PUT https://nms-dev.opk-bulat.ru/api/v1/catalog/default-configuration/3?indicator=2&device_component=4
...
```http
DELETE https://nms-dev.opk-bulat.ru/api/v1/catalog/default-configuration/3
...
> [!TIP]
> API предназначен для упорядочивания конфигураций по умолчанию в общем их перечне.
...
```http
PATCH https://nms-dev.opk-bulat.ru/api/v1/catalog/default-configuration/2/1
...
```http
GET https://nms-dev.opk-bulat.ru/api/v1/catalog/default-configurations
