...
func CreateDefaultConfiguration(c *gin.Context) {
	...
	ind, dc, _ := dao.GetDetailedDefaultConfigByID(c.Request.Context(), id)
	c.JSON(http.StatusCreated, model.DefaultConfiguration{ID: id, Indicator: ind, DeviceComponent: dc})
}
...
func GetDefaultConfiguration(c *gin.Context) {
	...
	ind, dc, err := dao.GetDetailedDefaultConfigByID(c.Request.Context(), id)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	c.JSON(http.StatusOK, model.DefaultConfiguration{ID: id, Indicator: ind, DeviceComponent: dc})
}
...
func UpdateDefaultConfiguration(c *gin.Context) {
	...
	ind, dc, _ := dao.GetDetailedDefaultConfigByID(c.Request.Context(), updatedID)
	c.JSON(http.StatusOK, model.DefaultConfiguration{ID: updatedID, Indicator: ind, DeviceComponent: dc})
}
...
// BindDefaultConfigurationThreshold связывает дефолтную конфигурацию с порогом
// @Summary         Связать дефолтную конфигурацию с порогом
// @Tags            9. Конфигурация: Конфигурации по-умолчанию
// @Accept          json
// @Produce         json
// @Param           request body dto.BindDefaultConfigThresholdRequest true "Данные связывания"
// @Success         200  {object}  map[string]string
// @Router          /api/v1/default-configurations/bind [post]
func BindDefaultConfigurationThreshold(c *gin.Context) {
	var input dto.BindDefaultConfigThresholdRequest
	if err := c.ShouldBindJSON(&input); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	err := dao.BindDefaultConfigurationThreshold(c.Request.Context(), input)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// UnbindDefaultConfigurationThreshold разрывает связь дефолтной конфигурации с порогом
...
func UnbindDefaultConfigurationThreshold(c *gin.Context) {
	defaultConfigID, errCfg := strconv.ParseInt(c.Param("defaultConfigurationId"), 10, 64)
	...
	found, err := dao.UnbindDefaultConfigurationThreshold(c.Request.Context(), defaultConfigID, thresholdID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
