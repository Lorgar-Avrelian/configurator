...
var tableLinks = []string{
	`ALTER TABLE public.logic_operator ADD CONSTRAINT "logic_operator_value_key" UNIQUE ("value")`,
...
func DropSchema(ctx context.Context) error {
	var err error
	var query string
...
func CreateSchema(ctx context.Context) error {
	...
	totalTables = len(freeTables)
	idx = totalTables
	for idx > 0 {
		idx--
		q = freeTables[totalTables-1-idx]
		wg.Add(1)
		go func(sqlQuery string) {
			var execErr error
			_, execErr = database.Get().Exec(ctx, sqlQuery)
			wg.Done()
			if execErr != nil {
				mu.Lock()
				if errResult == nil {
...
func CreateTableLinks(ctx context.Context) error {
	var err error
	var linkIdx int
	var linkQuery string
	var totalLinks int
	totalLinks = len(tableLinks)
	linkIdx = 0
	for linkIdx < totalLinks {
		linkQuery = tableLinks[linkIdx]
		_, err = database.Get().Exec(ctx, linkQuery)
		if err != nil {
			return err
		}
		linkIdx++
