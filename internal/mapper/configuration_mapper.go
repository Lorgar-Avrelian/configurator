...
func ConfigurationToConfigurationDto(rows []dao.Configuration) *dto.ConfigurationDto {
	if 0 >= len(rows) {
		return nil
	}
	var first dao.Configuration
	first = rows[0]
	var res dto.ConfigurationDto
	res.ID = first.ID
	var indDto dto.DeviceIndicatorDto
	indDto.ObjectID = first.IndicatorID
	...
	var i int
	var r dao.Configuration
	for i = 0; len(rows) > i; i++ {
		r = rows[i]
		processConfigComponentRow(r, dcMap, compMaps)
	}
	res.DeviceComponent = assembleConfigComponentTree(first.DeviceComponentID.Int64, dcMap, compMaps)
	return &res
}

func processConfigComponentRow(r dao.Configuration, dcMap map[int64]*dto.DeviceComponentDto, compMaps map[int64]map[int64]*dto.MappingDto) {
	...
	processConfigMappingRow(r, compMaps[dcID])
}

func processConfigMappingRow(r dao.Configuration, mapMap map[int64]*dto.MappingDto) {
	if !r.MapID.Valid {
		return
	}
	var mID int64
...
func assembleConfigComponentTree(rootID int64, dcMap map[int64]*dto.DeviceComponentDto, compMaps map[int64]map[int64]*dto.MappingDto) *dto.DeviceComponentDto {
	...
	for _, dc = range dcMap {
		...
		for _, m = range flatMaps {
			if m.From == nil {
				buildFlatConfigMappingTree(m, mapChildren)
	...
	buildFlatConfigComponentTree(root, childrenMap)
	return root
}

func buildFlatConfigComponentTree(parent *dto.DeviceComponentDto, childrenMap map[int64][]*dto.DeviceComponentDto) {
	...
	for i = 0; len(directChildren) > i; i++ {
		...
		buildFlatConfigComponentTree(child, childrenMap)
		valDc = *child
		parent.Children = append(parent.Children, valDc)
	}
}

func buildFlatConfigMappingTree(parent *dto.MappingDto, childrenMap map[int64][]*dto.MappingDto) {
	...
	for i = 0; len(directChildren) > i; i++ {
		child = directChildren[i]
		buildFlatConfigMappingTree(child, childrenMap)
		valM = *child
		parent.Children = append(parent.Children, valM)
	}
}

func ConfigurationArrayToConfigurationDtoArray(arr []dao.Configuration) []dto.ConfigurationDto {
	var groups map[int64][]dao.Configuration
	groups = make(map[int64][]dao.Configuration)
	var keys []int64
	keys = []int64{}
	var i int
	var r dao.Configuration
	...
	var res []dto.ConfigurationDto
	res = []dto.ConfigurationDto{}
	var k int64
	for _, k = range keys {
		var itemDto *dto.ConfigurationDto
		itemDto = ConfigurationToConfigurationDto(groups[k])
		if itemDto != nil {
